...
export const projects: Project[] = [
  ...
  },
  {
    ...
    longDescription: {
      fr: "Face à l'absence de gestion consolidée des impressions chez KISS, conception et déploiement de SavaPage comme gestionnaire d'impression centralisé, hébergé sur une VM Windows Server (Hyper-V). La solution centralise la file d'impression et le suivi des documents, garantissant une meilleure maîtrise des flux d'impression et une traçabilité complète dans l'entreprise.",
      en: "To address the lack of consolidated print management at KISS, designed and deployed SavaPage as a centralized print management solution, hosted on a Windows Server VM (Hyper-V). The solution centralizes print queues and document tracking, ensuring better control over print flows and full auditing across the company.",
    },
  ...
  },
  {
    ...
    longDescription: {
      fr: "Projet d'entraînement personnel, réalisé pour un client fictif (Stadium Company), de conception et déploiement d'une infrastructure IT complète. Architecture réseau avec redondance, mise en place de la virtualisation, configuration des services d'annuaire et de messagerie, et implémentation des politiques de sécurité conformes aux standards de l'industrie.",
      en: "Personal training project, carried out for a fictional client (Stadium Company), involving the design and deployment of a complete IT infrastructure. Redundant network architecture, virtualization rollout, directory and messaging services configuration, and security policies aligned with industry standards.",
    },
  ...
  },
  {
    ...
    longDescription: {
      fr: "Prise en charge d'un problème de couverture Wi-Fi historique (remontant à 2020) pénalisant le personnel soignant, connecté en Wi-Fi aux serveurs TSE (bureau à distance) pour utiliser l'application métier de gestion des dossiers patients. Diagnostic par tests de ping (comparaison PC-chariots / PC portables, sur batterie et sur secteur) et tests de roaming sur le terrain, ayant révélé de mauvaises configurations sur certaines bornes. Mise à jour des pilotes Wi-Fi via GPO et une solution de gestion de parc centralisée, vérification des règles de flux sur le pare-feu, puis bascule d'une partie des bornes de 2,4 GHz vers 5 GHz et ajustement du roaming, avec une amélioration significative de la couverture à la clé. Migration de la VM du serveur TSE vers des serveurs plus proches pour optimiser la communication.",
      en: "Took on a historic Wi-Fi coverage issue (dating back to 2020) affecting healthcare staff, who connect over Wi-Fi to TSE remote-desktop servers to use the patient records management application. Diagnosed the issue through ping tests (comparing PC-carts and laptops, on battery vs. mains power) and on-site roaming tests, which revealed misconfigurations on some access points. Updated Wi-Fi drivers via GPO and a centralized device management solution, checked firewall flow rules, then switched some access points from 2.4 GHz to 5 GHz and tuned roaming settings, achieving a significant coverage improvement. Migrated the TSE server VM to nearby servers to optimize communication.",
    },
