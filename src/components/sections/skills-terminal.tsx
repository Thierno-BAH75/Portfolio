...
function JsonView({ value }: { value: unknown }) {
  ...
  for (const match of text.matchAll(JSON_TOKEN)) {
    ...
    if (match[1] !== undefined) {
      nodes.push(
        <span key={key++} className={match[2] ? "text-cyan-300" : "text-violet-300"}>
          {match[1]}
        </span>
      );
...
export function SkillsTerminal({
  ...
  return (
    ...
    >
      {/* Fenêtre terminal — volontairement sombre dans les deux thèmes,
          accents violet/cyan du site */}
      <div className="rounded-2xl border border-border/60 bg-zinc-950 text-zinc-300 overflow-hidden shadow-[0_8px_40px_rgba(139,92,246,0.08)]">
        {/* Barre de titre */}
        <div className="flex items-center gap-3 px-4 py-2.5 border-b border-zinc-800 bg-zinc-900/80">
          ...
          <span className="font-mono text-xs text-zinc-400 truncate">{t.skills.terminal.title}</span>
          <span className="ml-auto flex items-center gap-1.5 text-[11px] font-medium text-green-400 whitespace-nowrap">
            <span className={cn("w-1.5 h-1.5 rounded-full bg-green-400", !reduce && "animate-pulse")} />
            {t.skills.terminal.online}
          </span>
        </div>

        {/* Endpoints cliquables */}
        <div className="px-4 pt-3 pb-2 border-b border-zinc-800/70">
          <p className="text-[11px] uppercase tracking-wider text-zinc-500 mb-2 font-mono">
          ...
          <div className="flex flex-wrap gap-1.5">
            {CATEGORIES.map((cat) => (
              <button
                ...
                className="font-mono text-xs px-2.5 py-1 rounded-md border border-zinc-700/80 text-zinc-300 hover:border-violet-500/60 hover:text-violet-300 hover:bg-violet-500/10 transition-colors disabled:opacity-50"
              >
                <span className="text-cyan-400">GET</span> /api/skills/{cat}
              </button>
            ))}
          </div>
        </div>
        ...
        >
          <p className="text-zinc-500">{t.skills.terminal.welcome1}</p>
          <p className="text-zinc-500">{t.skills.terminal.welcome2}</p>

          {entries.map((entry) => {
            if (entry.kind === "input") {
              return (
                <p key={entry.id} className="text-zinc-100">
                  <span className="text-cyan-400">visitor@portfolio</span>
                  <span className="text-zinc-500">:~$</span> {entry.text}
                </p>
              );
            }
            if (entry.kind === "error") {
              return (
                <p key={entry.id} className="text-red-400">
                  {entry.text}
                </p>
              );
            }
            ...
            return (
              <div key={entry.id} className="rounded-lg border border-zinc-800 bg-zinc-900/60">
                <div className="flex items-center gap-2 px-3 py-1.5 border-b border-zinc-800 text-[11px] flex-wrap">
                  <span className={entry.meta.status < 400 ? "text-green-400" : "text-red-400"}>
                    {entry.meta.status}
                  </span>
                  <span className="text-zinc-500 truncate">{entry.meta.url}</span>
                  {/* Temps réellement mesuré autour du fetch */}
                  <span className="text-violet-300">{entry.meta.ms} ms</span>
                  <button
                    type="button"
                    onClick={() => copyJson(entry.id, entry.json)}
                    className="ml-auto inline-flex items-center gap-1 text-zinc-400 hover:text-cyan-300 transition-colors"
                  >
                    {copiedId === entry.id ? <Check size={12} /> : <Copy size={12} />}
                    {copiedId === entry.id ? t.skills.terminal.copied : t.skills.terminal.copy}
                  </button>
                </div>
                <div className="px-3 py-2 max-h-64 overflow-y-auto text-zinc-300">
          ...
          {busy && <p className="text-zinc-500">…</p>}
        </div>

        {/* Ligne de commande */}
        <div className="flex items-center gap-2 px-4 py-3 border-t border-zinc-800 bg-zinc-900/50">
          <TerminalSquare size={14} className="text-violet-400 flex-shrink-0" aria-hidden="true" />
          ...
          <input
            ...
            spellCheck={false}
            enterKeyHint="send"
            className="flex-1 min-w-0 bg-transparent font-mono text-[13px] text-zinc-100 placeholder:text-zinc-600 focus:outline-none caret-cyan-400"
          />
        </div>
      </div>
    </motion.div>
