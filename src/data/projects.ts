...
export const projects: Project[] = [
  {
    ...
    description: {
      fr: "Dans le cadre du renforcement de l'infrastructure interne de KISS (ESN spécialisée en infogérance, hébergement et cybersécurité), conception et déploiement d'un datacenter interne réparti sur 2 baies : l'une dédiée à KHosting pour l'hébergement de clients stratégiques comme Foncia, l'autre aux environnements de production et préproduction internes de KISS. Câblage structuré avec code couleur par réseau (noir pour le management, gris pour la sauvegarde, rouge pour le WAN), switches Netgear stackés (haute disponibilité, bascule automatique), LAGs et liens SFP+ 10 Gb, VLANs segmentés (Management, Backup, WAN, DMZ, Interne, RDS). Serveurs Dell PowerEdge R6615 en RAID 10 (contrôleur Dell PERC H330 Mini), pare-feu pfSense, NAS Synology, hyperviseurs Proxmox et Hyper-V.",
      en: "As part of strengthening KISS's internal infrastructure (an IT services company specializing in managed services, hosting and cybersecurity), designed and deployed an in-house datacenter split across 2 racks: one dedicated to KHosting, hosting strategic clients such as Foncia, the other to KISS's internal production and pre-production environments. Structured cabling with a color code per network (black for management, grey for backup, red for WAN), stacked Netgear switches (high availability, automatic failover), LAGs and 10 Gb SFP+ links, segmented VLANs (Management, Backup, WAN, DMZ, Internal, RDS). Dell PowerEdge R6615 servers in RAID 10 (Dell PERC H330 Mini controller), pfSense firewalls, Synology NAS, Proxmox and Hyper-V hypervisors.",
    },
    longDescription: {
  ...
  },
  {
    ...
    longDescription: {
      fr: "Administration de l'infrastructure IT de l'Hôpital Franco-Britannique (270+ lits, environ 700 professionnels de santé), répartie sur 4 salles serveurs entre les sites de Kleber, Barbès et le nouveau site de Villiers. Mise en place de règles de filtrage et de flux (ACL) sur les pare-feux Fortigate, administration Active Directory (comptes utilisateurs, ordinateurs, lecteurs réseau, réservations DHCP) et Exchange pour plus de 500 utilisateurs, GPO d'automatisation et de sécurisation des postes, et déploiement SCCM avec masterisation des postes via Sysprep pour la gestion centralisée d'un parc de plusieurs centaines d'équipements. L'ensemble mené dans le respect des exigences RGPD propres au secteur de la santé.",
      en: "Administered the Hôpital Franco-Britannique's IT infrastructure (270+ beds, around 700 healthcare staff), spread across 4 server rooms between the Kleber, Barbès and new Villiers sites. Set up filtering and flow rules (ACLs) on Fortigate firewalls, administered Active Directory (user accounts, computers, network drives, DHCP reservations) and Exchange for 500+ users, built GPOs for workstation automation and hardening, and deployed SCCM with Sysprep-based masterization for centralized management of a fleet of several hundred devices. All carried out in line with the GDPR requirements specific to the healthcare sector.",
    },
